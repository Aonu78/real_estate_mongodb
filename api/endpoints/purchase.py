...
router = APIRouter(
    tags=['purchase near by zip_code/city/state'],
    prefix="/purchase"
)
@router.get("/purchase_by_zip")
async def property_by_zip(zipcode:str):
    db = property_db()
    # data = db["Details"].find({"zip_code" : zipcode,"site_type" : "rent","availability" : True,})
    data = db["Details"].find({"location.zip_code" : zipcode,"site_type" : "sale","availability" : True})
    lis = []
    for data in data:
...
@router.get("/purchase_by_city")
async def property_by_city(cityname:str):
    db = property_db()
    data = db["Details"].find({"location.city" : cityname,"site_type" : "sale","availability" : True})
    lis = []
    for data in data:
        data_id = propertiesEntity([data])
        propty: property_get.Property = property_get.Property(**data)
...
@router.get("/purchase_by_state")
async def property_by_state(statename:str):
    db = property_db()
    data = db["Details"].find({"location.state_code" : statename,"site_type" : "sale","availability" : True,})
...
@router.get("/purchase_by_country")
async def purchase_by_country(country:str):
    db = property_db()
    data = db["Details"].find({"location.country":country,"availability" : True,"site_type" : "sale"})
    lis = []
    for data in data:
        data_id = propertiesEntity([data])
        propty: property_get.Property = property_get.Property(**data)
